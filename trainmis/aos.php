...
<?php
	...
	if(count($arrnodeId) == count($arrmoduleName) && count($arrmoduleName) == count($arrmetaId)){
		for($i=0;$i<count($arrnodeId);$i++){
			...
			runMore($nodeId,$moduleName,$metaId,$atv,$i);
		}
	}else{
...
function runMore($nodeId,$moduleName,$metaId,$atv,$i){
	$ch = curl_init();  
	$timeout = 5;  
	$url="http://aos.corp.elong.com/apus_web/DeployInstance/GetVersions?nodeId=$nodeId&moduleName=$moduleName&allowThreeVersion=$atv&allowFourVersion=true";  
